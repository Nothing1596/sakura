...
本文定义待实现的语音输入能力。当前仓库已有 Plugin Runtime v4、TTS Hub 和插件输出 artifact，尚未交付本文的
ASR Hub、默认识别引擎、麦克风采集与录音界面。接口名称和 DTO 是本次设计草案，不能作为已经可调用的 SDK。
...
Host 音频输入能力以 `sakura.host.audio_input` 为设计边界；具体方法在实现时随类型化桥接一起确定。
...
普通状态下，麦克风紧邻发送按钮左侧。点击开始、再次点击结束，不要求持续按住，也不叠加长按语义。

| 状态 | 显示与操作 | 退出条件 |
|---|---|---|
| 可输入 | 原有输入框、麦克风、发送按钮 | 点击麦克风后检查引擎与设备 |
| 正在准备 | 简短状态文字，可取消；禁止重复开始和发送 | 引擎可用且麦克风成功打开后才进入录音 |
| 正在录音 | 左侧取消，中间波形与时长，原麦克风位置变为停止按钮；发送禁用 | 点击停止后识别，取消则丢弃本次录音 |
| 正在识别 | 状态文字和取消按钮；不再显示跳动波形，不允许再次录音或发送 | 成功回填，失败提示，或用户取消 |

录音布局参考用户提供的横向波形条：安静时是短线或圆点，有声音时出现上下对称的圆角竖条。正常输入框在录音
时临时由该区域替代，已有草稿保存在原会话中。识别时恢复草稿展示并暂时只读；完成或退出后恢复编辑与焦点。
...
- 第一版使用系统默认输入设备；每次开始录音重新读取默认设备，活动录音不自动切设备。设备断开、系统休眠或
  权限失败时终止本次录音并提示，恢复草稿，不自动识别不完整录音或重新收音。
- 第一版录音上限为 60 秒，达到上限后结束录音并识别，仍不发送。界面显示时长与上限；权限等待不计入时长。
- 没有可用 Hub、未选引擎或引擎缺少资源时，入口显示原因与配置入口，不先录音再让用户补装模型。
...
- 按钮提供可访问名称与键盘焦点，录音状态同时有文字和时长，不能只靠颜色或动画表达。
- 减少动态效果设置下使用静态状态和低频音量反馈。若波形绘制出现平台问题，可降级为时长与简单音量指示，
...
Hub 管理应用级 `selectedProviderId` 与默认识别语言，Provider 私有参数仍由各插件通过既有设置贡献管理。
设置页将“语音输入”和现有 TTS“语音输出”分开展示；输入引擎列表来自 Hub，不在前端维护模型名单。
...
下载进度、取消、错误和手动重试。普通启动和预热不静默下载。固定模型版本和校验值，安装完成后原子发布，
保留原可用版本直到替换成功。分发需记录代码、权重和 ONNX 转换产物各自的来源与许可。
...
本文尚无功能验收记录。文档检查通过只说明元数据、索引与链接有效，以下为后续实现需要取得的证据：
...
- 真实窗口验证波形随输入音量变化、安静基线、取消后停止、窄宽度、主题、DPI、键盘与减少动态效果；同时验证
  录音期间 TTS 不播放。Windows/macOS/Linux 的设备权限与打包能力分别记录，不用单平台证据替代其他平台。

行为测试沿真实调用链选择主要证据层；不单独断言 CSS 数值、竖条数量或完整提示文案。实现时再将新增测试加入
适当的 Harness profile，当前不宣称已有 ASR journey。
