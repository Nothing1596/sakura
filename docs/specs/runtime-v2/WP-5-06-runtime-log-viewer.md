...
- 查看器 DTO 使用 `schemaVersion: 3`，只能包含序号、`HH:MM:SS`、软件/TTS/插件 scope、等级、固定频道、
  ...
  稳定事件代码、固定中文消息或已清洗的自定义消息、warning/error 必有的固定中文说明、固定事件使用中文标签的安全详情，自定义事件使用有界字段名和值和最多一个 8 字符
...
- info/debug/trace 保持单行；带详情的条目以摘要行本身作为唯一展开入口，不再附加单独的“查看详情”行。warning 和 info
